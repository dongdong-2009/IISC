...
exports = module.exports = function(req, res) {
	...
	view.on('post', { action: 'visit' }, function(next) {
		...
		if(!req.body.email) {
			...
			updater.process(req.body, {
				flashErrors: true,
				fields: 'name, email,GID,depart,Other,location,ucard',
		...
		if (req.body &&req.body.depart&&req.body.depart!=28){
			req.body.Other='null';
		}
			...
			Visitor.model.findOne().where("email",req.body.email.toLowerCase()).exec(function(err, vistorItem) {
				...
				if(ifExist){
					...
					vc.exec(function(err,result){
						...
						updater.process(req.body, {
							flashErrors: true,
							fields: 'name, email,GID, depart,Other, location, ucard,v_id',
