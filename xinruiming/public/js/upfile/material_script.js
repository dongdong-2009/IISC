...
$("#uploadSubmit").on('click', function(evt) {
    ...
    console.log(file)
    console.log( !isValidPrjUrl(location.href))
   ...
   // xhr.open('post', '/project/uploadfile/' + prjid, true);
    ...
    xhr.open('post', '/template/uploadfile/create', true);
...
function deleteFile( fileId ){
    ...
    var url = '/template/deletefile/' + fileId;  
    var xhr = new XMLHttpRequest();  
...
function showPrjFiles() {
  ...
  var prjid = getUrlLastField(location.href);
  var url = '/template/listfile/' + "create";
  reqFileList(url);
...
function showFileList(fileListObj) {
  ...
  for(var i=0; i < fileListObj.length;i++ ) {
    ...
    console.log(fileListObj[i].userId);
    if(fileListObj[i].userId == "")
    {
	link.setAttribute("href", "/doc/visitor/"+fileListObj[i].saveName);
    }
    else{
    	link.setAttribute("href", "/doc/user_"+fileListObj[i].userId+"/prj_create"+"/"+fileListObj[i].saveName);
    }
