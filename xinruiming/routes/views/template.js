...
exports = module.exports = function(req, res) {
	...
	view.on('init', function (next) {
		...
		{
			createVisitorFolder();
			//createVisitorUploadFolder("create");
		}
		else
		{
...
exports.deletefile = function(req, res) {	
	...
	if(!req.user)	
	{
		var folder = getVisitorFolder();
	}
	else
	{
		var folder = getUploadFolder(req.user._id,"create");
	}
	var fileList = folder+"filelist.json";
	var file = folder+req.params.fileid;

	if(!req.params.fileid || !prjId) {
		res.status(400);
...
exports.uploadfile = function(req, res) {
	...
	if(!req.params.prjid || !req.files || !req.files.myFile) {
		res.status(400);
		res.end("Request URL Error");
		return;
	}
	if(!req.user){
		uploadFolder = getVisitorFolder();
	}
	else
	{
...
exports.listfile = function(req, res) {
	...
	if(!req.user)
	{
		var url = "/doc/visitor/filelist.json";	
	}
	else
	{
...
function createVisitorFolder() {

	var visitorDir = getVisitorFolder();
	try {
...
function createVisitorUploadFolder(prjId){

	var prjDir = getVisitorUploadFolder(prjId);
	var oldDir =getVisitorUploadFolder("create");
...
function getVisitorUploadFolder(){

	return getVisitorFolder() + "prj_" + "create" + "/";
}
...
function getVisitorFolder() {

	return path.join(__dirname, '../../public/doc/visitor')+ "/";
}
